...
class PGAgent(BaseAgent):
    ...
    def calculate_q_vals(self, rewards_list):
        ...
        if not self.reward_to_go:
            for t in rewards_list:
                q_values.append(self._discounted_return(t))
        ...
        # Estimate Q^{pi}(s_t, a_t) by the discounted sum of rewards starting from t
            ...
            # Done
            for t in rewards_list:
                q_values.append(self._discounted_cumsum(t))
            ...
            if self.gae_lambda is not None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    ## HINT: use terminals to handle edge cases. terminals[i]
                        ...
                        ## 0 otherwise.
                    if terminals[i] == 1: # state is the last in its trajectory
                         advantages[i] = rews[i] - values[i]
                    else:
                        dt = rews[i] + self.gamma*values[i+1] - values[i]
                        advantages[i] = dt + self.gamma * self.gae_lambda * advantages[i+1]
    ...
    def _discounted_return(self, rewards):
        ...
        """
        T = len(rewards)
        gammas = self.gamma**np.arange(T) # todo: check this
        discounted_return = np.sum(rewards * gammas)
        list_of_discounted_returns = np.repeat(discounted_return, T) # same reward for each timestep
    ...
    def _discounted_cumsum(self, rewards):
        ...
        list_of_discounted_cumsums = [np.sum(gammas[:T-t] * rewards[t:]) for t in range(T)]
